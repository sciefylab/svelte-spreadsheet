<script>
  ...
  export let position = { top: 0, left: 0 }; // Positioning
  ...
  let selectedDate = value && !isNaN(new Date(value + "T00:00:00").getTime()) ? new Date(value + "T00:00:00") : null;
  ...
  let currentMonth = initialViewDate.getMonth();
  let currentYear = initialViewDate.getFullYear();

  let focusedElement = null; // Track focused element for keyboard navigation
  let errorMessage = ""; // Error message state

  // Reactive statement to handle changes in the 'value' prop
  $: {
  ...
  function getDaysInMonth(year, month) {
    ...
    const daysArray = []; // Renamed to avoid conflict with Svelte's 'days'
    const startDay = firstDay.getDay(); // 0 (Sun) to 6 (Sat)
    const totalDaysInMonth = lastDay.getDate(); // Renamed
  ...
  // Reactive derived state for days in the current month
  ...
  function parseDate(input) {
    ...
    year = parseInt(year);
    if (year < 100) {
      // const currentYearValue = new Date().getFullYear(); // Not needed if regex ensures 4-digit year
  ...
  function selectDate(date) {
    ...
    onSelect(formatted);
    visible = false; // This will trigger reactivity if `visible` is bound in the parent
  }
...
{#if visible}
  <div
    class="spreadsheet-datepicker-popup fixed bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md shadow-lg z-[1000] p-2.5 w-[250px] font-sans focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400"
